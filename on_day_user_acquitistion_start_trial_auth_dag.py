...
doc_md = '''
# on_day_user_acquitistion_start_trial_auth DAG:

<p>Dag fully reload on-day uninstalls cohort, every day for last 60 days.</p>
...
- `kpis` - parameterized from `params.table_type` = `uninstalls`
...
**Note: in start_trial_auth cohorts column names starts with `Start Trial Auth` or `start trial auth` depends on platform, we replace original header with `start_trial_auth...` header.**
...
with DAG('on_day_user_acquisition_start_trial_auth',
         default_args=default_args,
         description='Daily reload start_trial_auth cohorts for last 60 days',
         max_active_runs=1,
         catchup=False,
         doc_md=doc_md,
         tags=['cohort', 'on_day', 'start_trial_auth', 'appsflyer'],
